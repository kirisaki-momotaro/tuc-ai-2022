...
class SequentialSearch(SearchBaseClass, ABC):
    ...
    def print_results(self,goal_node):
        f = open("output.txt", "a")
        ...
        for i in range(int(path_nodes_num)):
            print("(" + str(self.get_node_path(goal_node)[i][0]) + "," + str(self.get_node_path(goal_node)[i][1]) + ")", end="")
            ...
            if i < path_nodes_num - 1:
                ...
                print("")
                f.write("\n")
        print("\tHeuristic Cost (initial node):" + str(self.heuristic_function(self.initial_node)))
        f.write("\tHeuristic Cost (initial node):" + str(self.heuristic_function(self.initial_node))+"\n")
        print("\tEstimated Cost:" + str(self.heuristic_function(self.initial_node)))
        f.write("\tEstimated Cost:" + str(self.heuristic_function(self.initial_node))+"\n")
        print("\tReal Cost:" + str(self.cost_function(goal_node)))
        f.write("\tReal Cost:" + str(self.cost_function(goal_node)) + "\n")
        f.close()


    def recursive_search(self,limit,node_current):
        # parse through all successors available starting from the current_node
        if limit==0:
            self.initial_node=node_current
        ...
        if self.evaluation_function(node_current)<=limit:
            #new_limit used for comparison later to find the smaller cost
            new_limit=math.inf
            for primitive_successor in node_current.get_successors():
                ...
                child_cost,goal_found = self.recursive_search(limit,node_current=child)
                #trying to find the child with the smallest cost
                if(child_cost<new_limit):
                    new_limit=child_cost
        ...
        return self.evaluation_function(node_current),False
    ...
    def execute_search(self, time_pause) -> Tuple[Union[None, List[List[State]]], Union[None, List[MotionPrimitive]], Any]:
        ...
        found_path=False
        #f=self.evaluation_function(node_current=node_initial)

        #setting initial limit
        f=0
        #implement iterative deepening by calling a recursive func through a while loop
        while found_path==False:
            #new_f is the new limit corresponding to the child with the best heuristic
             new_f,found_path = self.recursive_search(f,node_current=node_initial)
             #print(new_f)
             f=new_f
